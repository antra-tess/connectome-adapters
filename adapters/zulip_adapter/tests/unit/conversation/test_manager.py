...
class TestManager:
    ...
    class TestGetOrCreateConversation:
        ...
        def test_get_conversation_id_private(self, manager, private_message_mock):
            """Test getting conversation ID for private message"""
            conversation_id = manager._get_conversation_id(private_message_mock)
            assert conversation_id == "101_102" # sorted user IDs

        def test_get_conversation_id_stream(self, manager, stream_message_mock):
            """Test getting conversation ID for stream message"""
            conversation_id = manager._get_conversation_id(stream_message_mock)
            assert conversation_id == "201/Test Topic"  # stream_id/topic

        def test_get_conversation_id_invalid(self, manager):
            """Test getting conversation ID for invalid message"""
            assert manager._get_conversation_id({}) is None

        def test_get_or_create_conversation_info_new(self, manager, stream_message_mock):
            """Test creating a new conversation"""
            assert len(manager.conversations) == 0
            conversation_info = manager._get_or_create_conversation_info(stream_message_mock)
        ...
        def test_get_or_create_conversation_info_existing(self, manager, private_message_mock):
            """Test getting an existing conversation"""
            manager._get_or_create_conversation_info(private_message_mock)
            conversation_info = manager._get_or_create_conversation_info(private_message_mock)
