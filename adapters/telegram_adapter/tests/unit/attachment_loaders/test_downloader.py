...
class TestDownloader:
    ...
    @pytest.fixture
    def downloader(self, client_mock, patch_config):
        """Create a Downloader with mocked dependencies"""
        yield Downloader(patch_config, client_mock)
    ...
    class TestDownloadAttachment:
        ...
        async def test_download_standard_file(self, downloader, mock_standard_file_message):
            ...
            with patch.object(downloader, "_get_attachment_metadata") as mock_get_metadata:
                ...
                with patch("os.path.exists", side_effect=[False, True]):  # File doesn't exist then does after download
                    with patch.object(downloader, "_create_attachment_dir") as mock_create_dir:
                        mock_create_dir.return_value = "test_dir"

                        with patch.object(downloader, "_download_standard_file") as mock_download:
                            with patch.object(downloader, "_create_metadata_file") as mock_create_metadata:
