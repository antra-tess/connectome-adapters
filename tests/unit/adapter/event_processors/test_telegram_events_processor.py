...
from adapter.event_processors.telegram_events_processor import TelegramEventsProcessor, EventType

class TestTelegramEventsProcessor:
    """Tests for the TelegramEventsProcessor class"""
    ...
    @pytest.fixture
    def processor(self, telethon_client_mock, conversation_manager_mock, config_mock):
        """Create a TelegramEventsProcessor with mocked dependencies"""
        with patch("adapter.event_processors.telegram_events_processor.Config") as ConfigMock:
            ConfigMock.return_value.get_instance.return_value = config_mock

            with patch("adapter.event_processors.telegram_events_processor.Downloader") as DownloaderMock:
                DownloaderMock.return_value = AsyncMock()
                DownloaderMock.return_value.download_attachment = AsyncMock(return_value={})
                processor = TelegramEventsProcessor(
    ...
    class TestProcessEvent:
        ...
        async def test_process_event_calls_correct_handler(self, processor, event_type):
            ...
            assert await processor.process_event(event_type, data) == ["event_info"]
        ...
        async def test_process_unknown_event(self, processor):
            ...
            result = await processor.process_event("unknown_event", MagicMock())
            assert result == []
        ...
        async def test_process_event_exception(self, processor, message_event_mock):
            ...
            with patch.object(processor, "_handle_new_message", side_effect=Exception("Test error")):
                result = await processor.process_event("new_message", message_event_mock)
                assert result == []
    ...
    class TestHandleNewMessage:
        ...
        async def test_handle_new_message(self, processor, message_event_mock, user_mock):
            ...
            result = await processor._handle_new_message(message_event_mock)
        ...
        async def test_handle_new_message_no_delta(self, processor, message_event_mock):
            ...
            processor.conversation_manager.create_or_update_conversation.return_value = None

            assert await processor._handle_new_message(message_event_mock) == []

        @pytest.mark.asyncio
        async def test_handle_new_message_no_message(self, processor):
            """Test handling a new message event with no message"""
            event = MagicMock()
            event.message = None

            assert await processor._handle_new_message(event) == []
        ...
        async def test_handle_new_message_exception(self, processor, message_event_mock):
            ...
            processor.conversation_manager.create_or_update_conversation.side_effect = Exception("Test error")
    ...
    class TestHandleDeletedMessage:
        ...
        async def test_handle_deleted_message_success(self, processor, deleted_message_event_mock):
            ...
            processor.conversation_manager.delete_from_conversation.return_value = "789"
        ...
        async def test_handle_deleted_message_no_conversation(self, processor, deleted_message_event_mock):
            ...
            processor.conversation_manager.delete_from_conversation.return_value = None
    ...
    class TestHandleChatAction:
        ...
        async def test_handle_pin_message(self, processor, pin_action_event_mock):
            ...
            result = await processor._handle_chat_action(pin_action_event_mock)
        ...
        async def test_handle_chat_action_unpin_message(self, processor, unpin_action_event_mock):
            ...
            result = await processor._handle_chat_action(unpin_action_event_mock)
