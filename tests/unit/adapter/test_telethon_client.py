...
class TestTelethonClient:
    ...
    @pytest.fixture
    def telethon_client(self, config_mock):
        """Create a TelethonClient with mocked dependencies"""
        with patch("adapter.telethon_client.Config") as ConfigMock:
            ConfigMock.return_value.get_instance.return_value = config_mock
            client = TelethonClient(AsyncMock())
            yield client
    ...
    class TestInitialization:
        ...
        def test_initialization_missing_credentials(self, config_mock):
            """Test initialization fails with missing API credentials"""
            config_mock.get_setting.side_effect = lambda section, key, default=None: {
                "telegram": {
                    "api_id": None,
                    "api_hash": None,
                }
    ...
    class TestConnection:
        ...
        async def test_connect_with_bot_token(self, telethon_client, telegram_client_mock):
            ...
            with patch("adapter.telethon_client.TelegramClient", return_value=telegram_client_mock):
                me = MagicMock()
