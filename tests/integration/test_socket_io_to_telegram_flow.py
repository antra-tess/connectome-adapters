...
from adapter.conversation_manager.conversation_data_classes import ConversationInfo
...
class TestSocketIOToTelegramFlowIntegration:
    ...
    @pytest.fixture
    def adapter(self, socketio_mock, telethon_client_mock):
        """Create a TelegramAdapter with mocked dependencies"""
        with patch("adapter.telethon_client.TelethonClient") as TelethonClientMock:
            TelethonClientMock.return_value = telethon_client_mock            
            adapter = TelegramAdapter(socketio_mock)
            adapter.socket_io_events_processor = SocketIoEventsProcessor(
                telethon_client_mock, adapter.conversation_manager
            )
            adapter.telegram_events_processor = TelegramEventsProcessor(
                telethon_client_mock, adapter.conversation_manager, "test_adapter"
            )
            yield adapter
    ...
    @pytest.mark.asyncio
    async def test_send_message_integration(self, adapter, telethon_client_mock, create_message_response):
        ...
        assert await adapter.socket_io_events_processor.process_event(
            "send_message", 
            {
                "conversation_id": "456",
                "text": "Hello, world!",
    ...
    @pytest.mark.asyncio
    async def test_edit_message_integration(self,
                                            adapter,
    ...
    @pytest.mark.asyncio
    async def test_delete_message_integration(self,
                                              adapter,
                                              telethon_client_mock,
                                              setup_conversation,
                                              setup_conversation_known_member,
    ...
    @pytest.mark.asyncio
    async def test_add_reaction_integration(self,
                                            adapter,
                                            telethon_client_mock, 
                                            setup_conversation,
                                            setup_conversation_known_member,
        ...
        with patch("adapter.event_processors.socket_io_events_processor.functions") as mock_functions, \
             patch("adapter.event_processors.socket_io_events_processor.ReactionEmoji") as mock_reaction_emoji:
    ...
    @pytest.mark.asyncio
    async def test_remove_reaction_integration(self,
                                               adapter,
                                               telethon_client_mock,
                                               setup_conversation,
                                               setup_conversation_known_member,
